...
train = full_cherry[c(1:62,74:135,146:207),]
validation = full_cherry[c(63:73,136:145,208:216),]

kyoto_model = full_cherry[full_cherry$location=="kyoto",]
mod = ets(kyoto_model[,9]) 
library(forecast)
test = forecast(mod, h=10)

preds_ky = data.frame(test)

liestal_model = full_cherry[full_cherry$location=="liestal",]
mod1 = ets(liestal_model[,9]) 
library(forecast)
test1 = forecast(mod, h=10)

preds_li = data.frame(test1)

washingtondc_model = full_cherry[full_cherry$location=="washingtondc",]
mod2 = ets(washingtondc_model[,9]) 
library(forecast)
test2 = forecast(mod2, h=10)

preds_wa = data.frame(test2)

rename(preds_ky, "kyoto" = "Point.Forecast")
rename(preds_li, "liestal" = "Point.Forecast")
rename(preds_wa, "washingtondc" = "Point.Forecast")

preds_final = data.frame(year=c(2022,2023,2024,2025,2026,2027,2028,2029,2030,2031),kyoto=preds_ky$Point.Forecast, liestal=preds_li$Point.Forecast, washingtondc=preds_wa$Point.Forecast)
